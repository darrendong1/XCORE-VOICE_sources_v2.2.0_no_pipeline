...
static int32_t Tile0_to_Tile1_i2s_audio[appconfAUDIO_PIPELINE_FRAME_ADVANCE][2];


#if ON_TILE(I2S_TILE_NO)
void i2s_rx_intertile(void *args) {
    (void) args;
...
int32_t mic_frame_data[MIC_ARRAY_CONFIG_MIC_COUNT][appconfAUDIO_PIPELINE_FRAME_ADVANCE]; //MIC_ARRAY_CONFIG_SAMPLES_PER_FRAME=128,MIC_ARRAY_CONFIG_MIC_COUNT=8
static void audio_mic_to_usb_i2s_process(void)
{
    static int mic_read_count = 0;
    size_t frame_count = 0;
    memset(mic_frame_data, 0x00, sizeof(mic_frame_data));

	for (;;) {
        int32_t **mic_ptr = (int32_t **)(mic_frame_data);  /* + (2 * frame_count)*/
...
/********************************usb record data  from DMIC*************************************/
        printf("usb_audio_send data ! \n");
        usb_audio_send(intertile_usb_audio_ctx,
                        frame_count,
                        mic_ptr,
                        8);
        printf("usb_audio_send data complete! \n");
...
//   
#if 0     
        int32_t *tmpptr = (int32_t *)mic_ptr;
        for (int j=0; j<frame_count; j++) {
            /* ASR output is first */
            Tile0_to_Tile1_i2s_audio[j][0] = *(tmpptr+j+(1*frame_count));    // ref mic0
            Tile0_to_Tile1_i2s_audio[j][1] = *(tmpptr+j+(2*frame_count));    // ref mic1
        }
                   

        rtos_intertile_tx(intertile_ctx, appconfI2S_OUTPUT_SLAVE_PORT, Tile0_to_Tile1_i2s_audio, appconfAUDIO_PIPELINE_FRAME_ADVANCE*2*4);
#endif
...
#if ON_TILE(I2S_TILE_NO) && appconfI2S_ENABLED 
    xTaskCreate((TaskFunction_t) i2s_rx_intertile,
                "i2s_slave_intertile",
                RTOS_THREAD_STACK_SIZE(i2s_rx_intertile),
                NULL,
                appconfAUDIO_PIPELINE_TASK_PRIORITY,
                NULL);
...
#if ON_TILE(MICARRAY_TILE_NO)
    printf("start audio_mic_to_usb_i2s_process on tile %d\n", THIS_XCORE_TILE);
    xTaskCreate((TaskFunction_t) audio_mic_to_usb_i2s_process,
                "audio_mic_to_usb_i2s_process",
                RTOS_THREAD_STACK_SIZE(audio_mic_to_usb_i2s_process),
                NULL,
                appconfAUDIO_PIPELINE_TASK_PRIORITY,
                NULL);
#endif
